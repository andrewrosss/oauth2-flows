...
import { setCookie, getCookies, deleteCookie } from "https://deno.land/std@0.120.0/http/cookie.ts";
  ...
  switch (`${method} ${pathname}`) {
    case "GET /": {
      if (access_token == null) {
        ...
        const repos = await api(`https://api.github.com/users/${username}/repos`);
        return page(
          <body>
            <div class="card">
              <p>
              ...
              <ul>
                {repos.map((repo: { full_name: string }) => <li><a class="link" href={`https://github.com/${repo.full_name}`}>{repo.full_name}</a></li>)}
              </ul>
            </div>
          </body>
        );
    ...
    case "GET /signout": {
      ...
      if (access_token != null) {
        ...
        await api(`https://api.github.com/applications/${Deno.env.get("CLIENT_ID")}/token`, { access_token });
      }
      return new Response(null, { status: 302, headers });
    }
...
const page = (body: Options["body"]) => html({
  title: "Oh Auth",
  links: [
...
const apiGet = (access_token: string) => (url: string) => fetch(
  url,
  {
    headers: {
...
const apiDelete = (access_token: string) => (url: string, body?: Record<string, any>) => fetch(
  url,
  {
    method: "DELETE",
    headers: {
